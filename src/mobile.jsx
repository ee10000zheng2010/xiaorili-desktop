...
const inRange = (task, day) =>
  task.date <= day && (task.endDate || task.date) >= day;
        ...
        {tab === "today" && (
          <section className="mobile-view">
            ...
            <div className="mobile-tip">
              <ListChecks size={15} />
              <span>未完成事项会持续显示，直到勾选完成或取消</span>
            </div>
          </section>
        )}
                    ...
                    >
                      ...
                      {items.length ? (
                        items.slice(0, 3).map((task) => (
                          <button key={task.id} onClick={(event) => { event.stopPropagation(); openEdit(task); }}>
                            {task.title}
                          </button>
                        ))
                      ) : (
              ...
              <div className="mobile-day-timeline">
                {selectedTasks.length ? (
                  selectedTasks.map((task) => (
                    <div className="mobile-timeline-row" key={task.id} onClick={() => openEdit(task)}>
